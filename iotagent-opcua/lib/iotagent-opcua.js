...
const opcuaBinding = require('./bindings/OPCUABinding');
const context = {
...
function deviceProvisioningHandler(device, callback) {
    transportSelector.applyFunctionFromBinding([device], 'deviceProvisioningHandler', null, function (error, devices) {
        if (error) {
            ...
            const provisionedDevice = devices[0];
            const endpoint = provisionedDevice.endpoint;

            if (device.context) {
                provisionedDevice.context = device.context;
            }

            if (provisionedDevice.internalAttributes.contexts) {
                const contexts = provisionedDevice.internalAttributes.contexts.map(ctx => ({
                    ...ctx,
                    endpoint: endpoint
                }));
                conf.iota.contexts = (conf.iota.contexts || []).concat(contexts);
            }

            if (provisionedDevice.internalAttributes.contextSubscriptions) {
                conf.iota.contextSubscriptions = (conf.iota.contextSubscriptions || []).concat(provisionedDevice.internalAttributes.contextSubscriptions);
            }

            if (provisionedDevice.internalAttributes.events) {
                conf.iota.events = (conf.iota.events || []).concat(provisionedDevice.internalAttributes.events);
            }

            config.setConfig(conf);
            opcuaBinding.startMonitoring();
            callback(null, provisionedDevice);
        }
    });
}
