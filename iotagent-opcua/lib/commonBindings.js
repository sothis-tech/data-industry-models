...
function singleMeasure(apiKey, deviceId, attribute, device, parsedMessage) {
    ...
    config.getLogger().debug(context, 'Sending update for device [%s] with context [%s] and type [%s]', device.name, device.context, device.type);
    iotAgentLib.update(device.name, device.type, device.context || '', values, device, function (error) {
