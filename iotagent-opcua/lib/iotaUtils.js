...
/**
 * Resolve the effective JSON-LD context for a device based on the hierarchy:
 * Device Context -> Group/Service Context -> Global Context.
 *
 * @param {Object} device    Device object.
 * @return {String}          The resolved context.
 */
function getEffectiveContext(device) {
    if (device && device.context) {
        return device.context;
    }
    if (device && device.jsonLdContext) {
        return device.jsonLdContext;
    }
    return config.getConfig().iota.globalContext || '';
}
    ...
    if (apiKey === config.getConfig().defaultKey) {
        iotAgentLib.getDevicesByAttribute('id', deviceId, null, null, function (error, devices) {
            if (error) {
                ...
                device.context = getEffectiveContext(device);
                callback(null, device);
            } else {
                config.getLogger().error(context, "MEASURES-001: Couldn't find device data for APIKey [%s] and DeviceId [%s]", apiKey, deviceId);

                callback(new errors.DeviceNotFound(deviceId));
            }
        });
    } else {
        async.waterfall([apply(iotAgentLib.getConfigurationSilently, config.getConfig().iota.defaultResource || '', apiKey), apply(findOrCreate, deviceId, transport), apply(iotAgentLib.mergeDeviceWithConfiguration, ['lazy', 'active', 'staticAttributes', 'commands', 'subscriptions'], [null, null, [], [], [], [], []])], function (error, device) {
            if (error) {
                callback(error);
            } else {
                device.context = getEffectiveContext(device);
